...
async def fetch_list_connections(tunnel_id: str):
    await http_cf_service.start_request()

    # Variables
    account_id = loaded_config['cloudflare']['account_id']

    response = await http_cf_service.session.get('/client/v4/accounts/' + account_id + '/cfd_tunnel/' + tunnel_id + '/connections')
